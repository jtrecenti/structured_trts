...
row = df[df.index == 6]
txt = row['txt_sentencas'].iloc[0]
result = extract_with_direct_api(txt, prompt, "llama-4-scout")
...
row = df[df.index == 3]
txt = row['txt_sentencas'].iloc[0]
result = extract_with_direct_api(txt, prompt, "gpt-4.1-mini")
print(result.model_dump_json())
```
...
row = df[df.index == 3]
txt = row['txt_sentencas'].iloc[0]
result = extract_with_direct_api(txt, prompt, "gemini-2.5-flash")
print(result.model_dump_json())
```
...
amostra = amostra.sample(n=100, random_state=42)
    ...
    for tipo in ['sentences_only']:
        ...
        if not f.exists():
            for model in available_models:
                ...
                result = extract_with_direct_api(txt, prompt, model)
