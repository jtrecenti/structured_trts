...
(
    ...
    p9.labs(
        ...
        x="Modelo",
        y="Custo médio por processo (USD)",
        fill="Provider"
    )
)
...
df_accuracy_cost = (
    ...
    # wrap 20 characters
    .assign(model_name = lambda x: x['model_name'].str.wrap(10))
)

df_accuracy_cost

(
    p9.ggplot(df_accuracy_cost, p9.aes(x='avg_cost', y='accuracy', color='provider')) +
    p9.geom_point(size=1) +
    p9.geom_text(p9.aes(label='model_name'), size=9) +
    p9.theme_minimal() +
    p9.scale_x_continuous(labels=lambda x: [f'${y:.2f}' for y in x]) +
    p9.scale_y_continuous(labels=lambda x: [f'{int(y*100)}%' for y in x]) +
    p9.scale_color_manual(values=['#1f77b4', '#ff7f0e', '#2c602c']) +
    p9.theme(legend_position='bottom') +
    p9.labs(
        subtitle="Acurácia X Custo",
        x="Custo médio (USD)",
        y="Acurácia (%)",
        color="Provider"
    )
